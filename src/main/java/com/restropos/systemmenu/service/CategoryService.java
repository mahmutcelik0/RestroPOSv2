...
public class CategoryService {
    ...
    public Category getCategoryByTitle(String categoryTitle, Workspace workspace) throws NotFoundException {
        return categoryRepository.getCategoryByCategoryTitleAndWorkspace(categoryTitle, workspace).orElseThrow(() -> new NotFoundException(CustomResponseMessage.CATEGORY_TITLE_NOT_FOUND));
    }

    public List<CategoryDto> getAllCategoriesForCustomer(String origin) {
        return categoryDtoPopulator.populateAll(categoryRepository.findAllByWorkspaceBusinessDomain(RequestUtils.getDomainFromOrigin(origin)));
    }
}
